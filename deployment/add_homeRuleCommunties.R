...
# MHVillage
mhvillage_df <- read.csv("data/MHVillage_IL_addedDistricts.csv")

boundaries <- read_sf('data/il_home_rule_municipalities.geojson')
colnames(boundaries)
...
final <- geocoded_final %>%
  st_drop_geometry() %>%
  distinct() # Flattens any 100% identical rows into a single row

write.csv(final, "data/MHvillage_IL_addedDistricts_addedHomeRuleMunicipalities.csv", row.names = FALSE)
